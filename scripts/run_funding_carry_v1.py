...
def _write_report(out: dict, py_pooled: dict) -> None:
    ...
    lines = [
        ...
        "",
        f"The pooled Sharpe below ({pm['sharpe']:.1f}) is high but **not a marking-frequency "
        "illusion** — the 8h realism pass (`funding_carry_realism_results.md`) re-marked the "
        "book on the funding-settlement grid and the Sharpe was unchanged (~8.6). The "
        "spot-perp basis is genuinely tight, so an *unlevered, fully-collateralized* "
        "delta-neutral carry really did earn ~14%/yr at low daily variance in 2020-2026. "
        "The danger is the **fat left tail, not the daily vol**: at 1x it is "
        "capital-inefficient, and any leverage introduces short-perp **liquidation in "
        "crashes** (the stress test goes negative at 3x and -90% at 10x). The Sharpe does "
        "not price that tail. **Trust the annual return and per-year regime picture; treat "
        "the Sharpe as a calm-regime figure that ignores the crash-liquidation tail.**",
        "",
        "## Headline (pooled book, base cost)",
        "",
        f"- Sharpe **{pm['sharpe']:.2f}** (calm-regime; ignores the leverage/liquidation "
        f"tail — see above), ann return **{pm['ann_return']*100:.2f}%**, ann vol "
        f"{pm['ann_vol']*100:.2f}%, max DD {pm['max_drawdown']*100:.1f}% (unlevered), "
    ...
    lines += [
        ...
        f"**{out['verdict']}.** " + (
            ...
            "and the gate (no-weakening) requires non-negative 2022 *and* 2026. The high "
            "Sharpe is real for an unlevered book but masks a crash-liquidation tail that "
            "turns the strategy negative under any leverage (see the realism pass). "
            "**DO_NOT_ADVANCE.** No paper/live."
        ),
    ]
    REPORT.write_text("\n".join(lines) + "\n")
